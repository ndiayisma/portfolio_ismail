...
import { GSBModal } from "../GSBModal";
import { ConventioModal } from "../ConventioModal";
import { CylisModal } from "../CylisModal";
import { ProjetPersonnelModal } from "../ProjetPersonnelModal";
...
export const Projects = () => {
    const [showGSBModal, setShowGSBModal] = useState(false);
    const [showConventioModal, setShowConventioModal] = useState(false);
    const [showCylisModal, setShowCylisModal] = useState(false);
    const [showProjetPersonnelModal, setShowProjetPersonnelModal] = useState(false);

    const handleGSBClick = (e) => {
        e.preventDefault();
        setShowGSBModal(true);
    };

    const handleConventioClick = (e) => {
        e.preventDefault();
        setShowConventioModal(true);
    };

    const handleCylisClick = (e) => {
        e.preventDefault();
        setShowCylisModal(true);
    };

    const handleProjetPersonnelClick = (e) => {
        e.preventDefault();
        setShowProjetPersonnelModal(true);
    };
    ...
    return (
        <section id="projects" className="min-h-screen flex items-center justify-center px-4 py-16 relative">
            ...
            <div className="max-w-4xl relative z-10">
                ...
                <div className="grid md:grid-cols-2 gap-8 mt-12">
                    {projects.map((project, index) => {
                        const isGSB = project.title === "GSB-Express";
                        const isConventio = project.title === "Projet Conventio";
                        const isCylis = project.title === "Cylis";
                        const isProjetPersonnel = project.title === "Projet Personnel";
                        
                        if (isGSB || isConventio || isCylis || isProjetPersonnel) {
                            let onClick;
                            if (isGSB) onClick = handleGSBClick;
                            if (isConventio) onClick = handleConventioClick;
                            if (isCylis) onClick = handleCylisClick;
                            if (isProjetPersonnel) onClick = handleProjetPersonnelClick;
            ...
            <GSBModal isOpen={showGSBModal} onClose={() => setShowGSBModal(false)} />
            <ConventioModal isOpen={showConventioModal} onClose={() => setShowConventioModal(false)} />
            <CylisModal isOpen={showCylisModal} onClose={() => setShowCylisModal(false)} />
            <ProjetPersonnelModal isOpen={showProjetPersonnelModal} onClose={() => setShowProjetPersonnelModal(false)} />
        </section>
    );
}